...
public class JugadorResource {
    ...
    @RequestMapping(value = "/topScore/{canastas}",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<List<Jugador>> topScore(@PathVariable Integer canastas, Pageable pageable)
        throws URISyntaxException {
        Page<Jugador> page = jugadorRepository.topScore(canastas, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/jugadors/topScore");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }

    @RequestMapping(value = "/Asist/{totalAsist}",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<List<Jugador>> Asist(@PathVariable Integer totalAsist, Pageable pageable)
        throws URISyntaxException {
        Page<Jugador> page = jugadorRepository.Asist(totalAsist, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/jugadors/Asist");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
    ...
    @RequestMapping(value = "/_search/jugadors/{query:.+}",
        ...
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public List<Jugador> searchJugadors(@PathVariable String query) {
        log.debug("REST request to search Jugadors for query {}", query);
        return StreamSupport
            .stream(jugadorSearchRepository.search(queryStringQuery(query)).spliterator(), false)
